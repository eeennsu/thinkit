...
여덟 개 질문. 각각이 여기 있는 이유는 그 답을 레포를 읽어서 찾을 수 없기 때문이다 —
Q0과 마지막 두 개는, 레포가 보여주는 것이 과거에 대한 서술일 뿐이고 앞으로의 규칙으로
...
마지막 두 개는 Q0이 고른 모듈의 레이어 그래프가 `variants` 아래 선언한 값 중 하나를
받는다. 모르는 값은 실행을 멈추고, 대체 기본값은 없다. 오타가 조용히 엄격한 설정을
만들어내면 그것이 레포의 답으로 보고되기 때문이다. 경계를 강제하지 않는 모듈을 골랐으면
그 둘은 묻지 않는다 — 조일 것이 없는데 도착한 답도 같은 이유로 실행을 멈춘다.

---

## Q0 — 이 레포에 레이어 경계 규약이 있는가? 무엇이 그것을 강제하는가?

```yaml
id: Q0
axis: principle
key: architecture
values: modules/ 아래 모듈 이름 (`none` 포함)
writes: 경계 정책이 생성되는지 여부, 그리고 Q6·Q7이 물어지는지 여부
default: 스택 프로필의 architecture를 제시하고 확인받는다
```

스택 프로필이 하나를 지목하지만 그것은 제안이지 답이 아니다. 스택은 프레임워크가 무엇을
가져오는지를 알고, 이 레포가 어떻게 나뉘어 있는지는 모른다.

먼저 읽는다. Q4·Q6와 같은 방식이다.

```
레이어 이름이 디렉터리로 있나   ls <sourceRoot>
이미 강제하는 것이 있나         eslint 설정에 boundaries / import/no-restricted-paths / dependency-cruiser
```

레이어 이름이 그대로 있으면 그 모듈을 제시하고 확인받는다. 아무것도 없으면 `none`을
제시한다. **없는 규약을 세워 주지 않는다.** 우리가 아는 모듈로 서술되지 않는 자기 규약을
가진 레포도 `none`이다 — 틀린 그래프는 없는 그래프보다 나쁘고, 그런 레포는 언젠가 자기
모듈을 받는다.

답이 `none`이면: 경계 설정을 쓰지 않고, 레이어 디렉터리를 만들지 않고, 경계 플러그인을
설치 목록에 넣지 않는다. 나머지 하네스 — 산문, 포매터, tsconfig, 심어진 체커 — 는 그대로
나가고, 보고서는 강제되는 경계를 0으로 세면서 그것이 답이라고 적는다. 구멍이 아니다.
...
```
index가 있는 슬라이스 루트   find <sourceRoot>/{entities,features,widgets} -maxdepth 2 -name 'index.*'
...
`routingRoot`는 `sourceRoot` 안에 있을 수도(`src/navigators/`) 옆에 있을 수도(`app/`) 있다.
둘 다 된다. 라우팅 요소가 레이어보다 먼저 등록되므로 `sourceRoot` 안에 있어도 라우팅으로
매칭된다.
