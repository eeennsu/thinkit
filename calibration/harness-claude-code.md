...
아직 소비하는 규칙이 없다. `claude-md.memory-log`가 전제로 삼는 것이 이 값이지만 그 규칙은
`axis: principle`이고, `check.mjs`는 `axis: calibrated`인 규칙에만 캘리브레이션을 읽힌다.
소비자가 붙기 전까지 이 값은 사람이 읽는 기록이다.
