...
레포의 하네스 — `CLAUDE.md`, 스킬, 레퍼런스, 린트 경계 — 를 세팅하고 같은 기준으로 계속
감사하는 Claude Code 플러그인.
...
형태로 묶은 것이다. **CLAUDE.md를 만들어주는 도구가 아니라, 계속 얇게 유지시켜주는 도구다.**
...
**CLAUDE.md** — 얇다. 얇은 게 성공이다. 대신 완료 보고가 산출물 목록으로 나온다: 생성된 파일,
도구가 강제하는 규칙 수, CLAUDE.md 근사 토큰, check 통과 여부.
...
`no-unused-vars`(`^_`로 예외). 전부 "작동하는 코드처럼 보이는 실수"고, 안 넣으면 CLAUDE.md에
...
세팅 직후 CLAUDE.md의 Gotchas 섹션은 비어 있다. 의도한 것이다. gotcha는 정의상 레포 안에서
