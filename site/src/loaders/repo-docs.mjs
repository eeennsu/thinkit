...
export const REPO_ROOTS = [
  {
    dir: "docs",
    // 둘 다 빠지지만 이유는 다르다. 한 배열에 있다고 같은 판정이 아니다.
    //
    // design-log.md — 판정 기준이 아니라 작업 로그다. 사이트의 일은 이 플러그인이 무엇인지
    //   설명하는 것이고, 날짜가 박힌 개발 일지는 그 일을 하지 않으면서 사이드바 한 자리를
    //   차지한다.
    // calibration-notes.md — 제목이 말하는 그대로 배선되지 않은 관찰이다. 아무 스킬도
    //   스크립트도 이 값을 읽지 않는다. 발행하면 판정 기준 문서와 같은 층에 앉아, 사이트가
    //   "이 값이 판정에 쓰인다"고 말하는 것으로 읽힌다. 소비자가 생기는 순간 그 값은
    //   출처와 함께 calibration/ 으로 옮겨가고, 그때 발행되는 것은 이 파일이 아니다.
    //
    // 근거를 찾는 사람은 README와 랜딩의 링크에서 레포로 간다.
